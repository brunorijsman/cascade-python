...
def test_create_shuffle_from_identifier():
    ...
    key = Key()
    original_shuffle = Shuffle(key.size, Shuffle.SHUFFLE_KEEP_SAME)
    recreated_shuffle = Shuffle.create_shuffle_from_identifier(original_shuffle.identifier)
    assert original_shuffle.__repr__() == recreated_shuffle.__repr__()

    # Empty random shuffle
    key = Key()
    original_shuffle = Shuffle(key.size, Shuffle.SHUFFLE_RANDOM)
    recreated_shuffle = Shuffle.create_shuffle_from_identifier(original_shuffle.identifier)
    assert original_shuffle.__repr__() == recreated_shuffle.__repr__()
...
def test_identifier():
    ...
    key = Key.create_random_key(8)
    shuffle = Shuffle(key.size, Shuffle.SHUFFLE_RANDOM)
    assert shuffle.identifier == 18048933084901000000008

...
def test_key_16_block_4_window_2():
    run_eve_absent_scenario(key_size=16, block_size=4, window_size=2)

def test_key_64_block_32_window_8():
    run_eve_absent_scenario(key_size=64, block_size=32, window_size=8)

def test_key_16_block_4_window_1():
    # Window size is degenerate value of 1
    run_eve_absent_scenario(key_size=16, block_size=4, window_size=1)

def test_key_16_block_4_window_4():
    # Block size is same as window size
    run_eve_absent_scenario(key_size=16, block_size=4, window_size=4)

def test_key_8_block_8_window_4():
    # Key size is same as block size
    run_eve_absent_scenario(key_size=8, block_size=8, window_size=4)

def test_key_8_block_8_window_8():
    # Key size, block size, and window size are all the same
    run_eve_absent_scenario(key_size=8, block_size=8, window_size=8)

def test_key_27_block_8_window_4():
    # Key size is not a multiple of block size
    run_eve_absent_scenario(key_size=27, block_size=8, window_size=4)
